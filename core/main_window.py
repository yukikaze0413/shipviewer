...
class MainWindow(QMainWindow):
    ...
    def _load_damage_tree_from_csv(self, csv_path=None):
        ...
        self._damage_children_by_id = defaultdict(list, {key: list(value) for key, value in children_map.items()})
    ...
    def _load_gltf_with_importer(self, filepath):
        ...
        try:
            ...
            while True:
                ...
                imported_actors.append((actor, points, cells, self._vtk_object_name(actor), actor.GetBounds()))

            display_names = self._match_gltf_display_names(
                gltf_name_entries,
                [(points, cells, bounds) for _actor, points, cells, _actor_object_name, bounds in imported_actors],
            )
    ...
    def _attach_gltf_reader_bounds(self, filepath, name_entries):
        ...
        for entry_key in ("node", "primitive"):
            ...
            for entry, reader_entry in zip(entries, reader_entries):
                if (
                    entry.get("points") == reader_entry.get("points")
                    and entry.get("cells") == reader_entry.get("cells")
                ):
    ...
    def _match_gltf_display_names(self, name_entries, actor_stats):
        ...
        for actor_idx, actor_stat in enumerate(actor_stats):
            ...
            entry_idx = self._best_gltf_match_entry_index(exact_indexes, match_entries, bounds)
    ...
    def _best_gltf_match_entry_index(self, exact_indexes, match_entries, actor_bounds):
        ...
        for entry_idx in exact_indexes:
            ...
            if entry_bounds is None:
                continue
            scored.append((self._bounds_center_distance(actor_bounds, entry_bounds), entry_idx))
        if not scored:
            return exact_indexes[0]
    ...
    def _damage_display_matches(self, node_matches, object_matches):
        ...
        for row in (*node_matches, *object_matches):
            key = (row.get("model_name"), row.get("damage_leaf_id"), row.get("document_path"))
            if key in seen:
                continue
